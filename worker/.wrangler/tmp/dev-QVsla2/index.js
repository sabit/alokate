...
var e = /* @__PURE__ */ __name(({ base: e2 = "", routes: t = [] } = {}) => ({ __proto__: new Proxy({}, { get: /* @__PURE__ */ __name((o2, s2, r, n) => (o3, ...a) => t.push([s2.toUpperCase(), RegExp(`^${(n = (e2 + o3).replace(/\/+(\/|$)/g, "$1")).replace(/(\/?\.?):(\w+)\+/g, "($1(?<$2>*))").replace(/(\/?\.?):(\w+)/g, "($1(?<$2>[^$1/]+?))").replace(/\./g, "\\.").replace(/(\/?)\*/g, "($1.*)?")}/*$`), a, n]) && r, "get") }), routes: t, async handle(e3, ...o2) {
  let s2, r, n = new URL(e3.url), a = e3.query = { __proto__: null };
  for (let [e4, t2] of n.searchParams) a[e4] = void 0 === a[e4] ? t2 : [a[e4], t2].flat();
  for (let [a2, c2, l2, i2] of t) if ((a2 === e3.method || "ALL" === a2) && (r = n.pathname.match(c2))) {
    e3.params = r.groups || {}, e3.route = i2;
    for (let t2 of l2) if (void 0 !== (s2 = await t2(e3.proxy || e3, ...o2))) return s2;
  }
} }), "e");
...
// src/schemas.ts
var authSchema = z.object({ pin: z.string().min(4) });
...
var snapshotDataSchema = z.object({
  config: configSchema,
...
var unifiedStateSchema = z.object({
  config: configSchema,
  preferences: preferenceSchema,
  schedule: z.array(scheduleEntrySchema),
...
// src/storage.ts
var DEFAULT_UNIFIED_STATE = unifiedStateSchema.parse({
  config: {
    faculty: [],
    subjects: [],
    sections: [],
  ...
  snapshots: [],
  settings: {
    weights: { mobility: 0.8, seniority: 1.2, preference: 1 },
    theme: "dark",
    optimizerSeed: 42
  }
});
var metadataFromSnapshot = /* @__PURE__ */ __name((snapshot) => ({
  id: snapshot.id,
...
var LIST_SNAPSHOTS_SQL = "SELECT id, snapshot_name AS snapshotName, timestamp, hash FROM snapshots ORDER BY datetime(timestamp) DESC";
var UPSERT_STATE_SQL = "INSERT INTO unified_state (id, data, updated_at) VALUES (?, ?, ?) ON CONFLICT(id) DO UPDATE SET data = excluded.data, updated_at = excluded.updated_at";
var UPSERT_SNAPSHOT_SQL = "INSERT INTO snapshots (id, snapshot_name, timestamp, hash, data) VALUES (?, ?, ?, ?, ?) ON CONFLICT(id) DO UPDATE SET snapshot_name = excluded.snapshot_name, timestamp = excluded.timestamp, hash = excluded.hash, data = excluded.data";
var SELECT_SNAPSHOT_SQL = "SELECT id, snapshot_name AS snapshotName, timestamp, hash, data FROM snapshots WHERE id = ?";
var STATE_EXISTS_SQL = "SELECT 1 FROM unified_state WHERE id = ?";
var listSnapshotMetadata = /* @__PURE__ */ __name(async (env) => {
  if (env.DB) {
    const { results } = await env.DB.prepare(LIST_SNAPSHOTS_SQL).all();
...
var getUnifiedState = /* @__PURE__ */ __name(async (env) => {
  if (env.DB) {
    const row = await env.DB.prepare("SELECT data FROM unified_state WHERE id = ?").bind("primary").first();
    if (row?.data) {
      try {
...
var saveUnifiedState = /* @__PURE__ */ __name(async (env, state) => {
  ...
  if (env.DB) {
    await env.DB.prepare(UPSERT_STATE_SQL).bind("primary", serialized, timestamp).run();
  } else {
    memoryStore.state = clone(parsed);
    memoryStore.initialized = true;
  }
}, "saveUnifiedState");
var saveSnapshotRecord = /* @__PURE__ */ __name(async (env, input) => {
  const snapshot = snapshotSchema.parse(input);
  const payload = snapshot.data ? JSON.stringify(snapshot.data) : null;
  if (env.DB) {
    await env.DB.prepare(UPSERT_SNAPSHOT_SQL).bind(snapshot.id, snapshot.snapshotName ?? null, snapshot.timestamp, snapshot.hash ?? null, payload).run();
  } else {
...
var getSnapshotRecord = /* @__PURE__ */ __name(async (env, id) => {
  if (env.DB) {
    const row = await env.DB.prepare(SELECT_SNAPSHOT_SQL).bind(id).first();
    if (!row) {
      return null;
    }
    return {
      id: row.id,
      snapshotName: row.snapshotName ?? void 0,
...
var diffSnapshots = /* @__PURE__ */ __name(async (env, { fromId, toId }) => {
...
var ensureDefaultState = /* @__PURE__ */ __name(async (env) => {
  if (env.DB) {
    const existing = await env.DB.prepare(STATE_EXISTS_SQL).bind("primary").first();
    if (!existing) {
...
var requireBearer = /* @__PURE__ */ __name((request) => {
...
router.post("/data", async (request, env) => {
  const authError = requireBearer(request);
  if (authError) {
    return authError;
  }
...
router.post("/snapshot", async (request, env) => {
  const authError = requireBearer(request);
  if (authError) {
    return authError;
  }
...
router.get("/snapshot/:id", async (request, env) => {
  const authError = requireBearer(request);
  if (authError) {
    return authError;
  }
...
router.post("/diff", async (request, env) => {
  const authError = requireBearer(request);
  if (authError) {
    return authError;
  }
...
router.post("/settings", async (request, env) => {
  const authError = requireBearer(request);
  if (authError) {
    return authError;
  }
...
// .wrangler/tmp/bundle-3aBAt1/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-3aBAt1/middleware-loader.entry.ts
